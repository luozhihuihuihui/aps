...
import com.hithy.auth.util.SpringBeanUtil;
import com.hithy.auth.util.common.JsonConvertUtil;
...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        if (this.isLoginAttempt(request, response)) {
            ...
            // tokenCheck = true 开启任何请求必须登录才可访问
            if (SpringBeanUtil.getBean(RedisConfig.class).getTokenCheck()) {
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        if (redisUtil.exists(ApsConstant.Redis.PREFIX_APS_REFRESH_TOKEN + account)) {
            ...
            if (JwtUtil.getClaim(token, ApsConstant.JWT.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                redisUtil.setObject(ApsConstant.Redis.PREFIX_APS_REFRESH_TOKEN + account, currentTimeMillis, Integer.parseInt(refreshTokenExpireTime));
    ...
    private void response401(ServletResponse response, String msg) {
        ...
        try (PrintWriter out = httpServletResponse.getWriter()) {
            ApsResponse error = ApsResponse.unauthorized("Unauthorized:" + msg);
            String data = JsonConvertUtil.objectToJson(error);
            out.append(data);
        } catch (IOException e) {
            log.error("直接返回Response信息出现IOException异常:{}", e.getMessage());
            throw new ApsException("直接返回Response信息出现IOException异常:" + e.getMessage());
        }
    }
}
