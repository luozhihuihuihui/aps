...
import com.hithy.auth.util.common.Base64ConvertUtil;
import com.hithy.auth.util.common.HexConvertUtil;
...
public class AesCipherUtil {
    private static RedisConfig redisConfig = SpringBeanUtil.getBean(RedisConfig.class);
    /**
     * logger
     */
    ...
    public static String enCrypto(String str) {
        try {
            ...
            secureRandom.setSeed(Base64ConvertUtil.decode(redisConfig.getEncryptAESKey()).getBytes());
            ...
            throw new ApsUnauthorizedException("getInstance()方法异常:" + e.getMessage());
        } catch (UnsupportedEncodingException e) {
            logger.error("Base64加密异常:{}", e.getMessage());
            throw new ApsUnauthorizedException("Base64加密异常:" + e.getMessage());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:{}", e.getMessage());
            throw new ApsUnauthorizedException("初始化Cipher对象异常:" + e.getMessage());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("加密异常，密钥有误:{}", e.getMessage());
            throw new ApsUnauthorizedException("加密异常，密钥有误:" + e.getMessage());
        }
    }
    ...
    public static String deCrypto(String str) {
        try {
            ...
            throw new ApsUnauthorizedException("Base64解密异常:" + e.getMessage());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:{}", e.getMessage());
            throw new ApsUnauthorizedException("初始化Cipher对象异常:" + e.getMessage());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("解密异常，密钥有误:{}", e.getMessage());
            throw new ApsUnauthorizedException("解密异常，密钥有误:" + e.getMessage());
        }
    }
}
